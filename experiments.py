...
if "gossip" in scheme:
    ...
    devices = {u: gossip.PageRankDevice(u, onehot_labels[u], onehot_labels[u] if u in training else empty_label) for u in G}
else:
...
for epoch in range(100):
    ...
    accuracy = sum(1. if devices[u].predict(False) == labels[u] else 0 for u in test) / len(test)
