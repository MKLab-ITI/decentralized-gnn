...
def create_network(
    ...
    **kwargs
    ...
    network = Network(
        graph,
        lambda u: device_type(
            u,
            init_classifier(u),
            features[u],
            training_labels[u],
            train_steps=1 if u in training else 0,
            **kwargs
        ),
        seed=seed,
...
class Network:
    def __init__(
        ...
        self.neighbors = {
            u: {
                v: random.random() * (max_communication_rate - min_communication_rate)
                + min_communication_rate
        ...
        self.device_list = list(
            self.devices.values()
        )  # values of self.devices stored as a list
    ...
    def round(self):
        experiment_edges = [
            (u, v)
            for u, neighbors in self.neighbors.items()
            for v, freq in neighbors.items()
